...
const walletKeyboard = {
  keyboard: [[{ text: "USD" }, { text: "EUR" }], [{ text: "Попереднє меню" }]],
  one_time_keyboard: true,
};
let currentCity = "";
...
async function getCurrencyUSD() {
  try {
    ...
    const filteredResponse = response.data.filter(
      (obj) => obj.currencyCodeA === 840 || obj.currencyCodeB === 840
    );
...
async function getCurrencyEUR() {
  try {
    const response = await axios.get("https://api.monobank.ua/bank/currency");
    const filteredResponse = response.data.filter(
      (obj) => obj.currencyCodeA === 978 || obj.currencyCodeB === 978
    );
    return filteredResponse;
  } catch (error) {
    console.error("An error occurred:", error.message);
...
bot.onText(/\/start/, (msg) => {
  ...
  bot.sendMessage(
    chatId,
    "Привет! Нажмите кнопку 'Погода' или 'Курс валют' для начала.",
    {
...
bot.on("message", async (msg) => {
    ...
    try {
      ...
      currencyRates.forEach((rate) => {
        if (rate.currencyCodeA === 840 || rate.currencyCodeB === 840) {
          message += `${
            rate.currencyCodeA === 840 ? rate.currencyCodeB : rate.currencyCodeA
          }: ${rate.rateBuy} (Покупка), ${rate.rateSell} (Продажа)\n`;
        }
      });
    ...
    try {
      ...
      currencyRates.forEach((rate) => {
        if (rate.currencyCodeA === 978 || rate.currencyCodeB === 978) {
          message += `${
            rate.currencyCodeA === 978 ? rate.currencyCodeB : rate.currencyCodeA
          }: ${rate.rateBuy} (Покупка), ${rate.rateSell} (Продажа)\n`;
        }
      });
  ...
  if (msg.text === "Погода") {
    ...
    if (!currentCity) {
      bot.sendMessage(
        chatId,
        "Сначала установите город с помощью команды /setcity <город>"
      );
      return;
    }
    ...
    try {
      ...
      const forecasts = await getWeatherForecastByCoordinates(
        lat,
        lon,
        weatherApiKey
      );
      ...
      bot.sendMessage(
        chatId,
        `Не удалось получить данные о погоде для ${currentCity}. Проверьте название города и попробуйте снова.`
      );
    }
    ...
    if (currentCity) {
      ...
      bot.sendMessage(
        chatId,
        "Город не установлен. Используйте команду /setcity <город>, чтобы установить город."
      );
    }
